...
\noindent Schauen wir uns nun die Multinomialeverteilung an. Wir gehen ähnlich vor wie bei der Zero-Inflated negativ Binomialverteilung vor und stellen die Anzahl der korrekten Vorhersagen gegenüber der Anzahl der ground truth (pro label). Um dies mit der Zero-Inflated negativ Binomialverteilung vergleichen zu können, skalieren wir die Vorhersage der Zero-Inflated negativ Binomialverteilung logarithmisch und stellen dies ebenfalls als Histogramm dar. Die folgende Abbildung zeigt das Ergebnis dieser Vorgehensweise.\\
...
\end{tabular}
\caption{Histogramm der Vorhersagen für beide Verteilungen und Architekturen. Die erste Zeile entspricht der Zero-Inflated negativ Binomialverteilung, die letzte der Multinomialverteilung \label{fig:logDist}}
\end{figure}

\noindent Anhand des Histogramms ist zu erkennen, dass die Zero-Inflated negativ Binomialverteilung eine leicht bessere Vorhersage liefert. Das Histogramm wurde in Richtung y-achse bei 0.2 begrenzt um eine bessere Darstellung zu erreichen. Stellt man die Beziehung in einer Tabelle dar, so kommt man zu folgendem Ergebnis.\\
